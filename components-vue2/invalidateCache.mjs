  ...
  fs.readdir(pathToDirectory, { withFileTypes: true }, async (error, files) => {
    ...
    try {
      for await (const menu of directoriesInDIrectory) {
        const settings = await fetch(encodeURI(`${BASE_URL}/${URLS.CLIENTFREEMENU}/55/${menu}`));
        const list = await fetch(encodeURI(`${BASE_URL}/${URLS.FREEDATA}/55/${menu}/0/0`));
        if (list.ok) {
          ...
          const dataList = {
            ...
            subSettings: jsonSettings ? menuConverter.menuObject(jsonSettings[0]) : undefined,
          };
          const { REL, ID } = dataList.items[0];
          const card = await fetch(encodeURI(`${BASE_URL}/${URLS.FREEDATACARD}/55/${menu}/${ID}?REL=${REL}`));
          if (card.ok) {
            const mobile2ServiceInstance = mobile2Service();
            const jsonCard = await card.json();
            const data = await formConverter.form(jsonCard, { idItem: menu, zone: "free" }, mobile2ServiceInstance);
