...
async function eventHandler(fields, action, func) {
  ...
  const isVehicleModel = fields.find((f) => f.name === "SVEHICLE_MODEL");

  const errRegNumNotFound = fields.find(
    (f) => f.name === "input_label_err_regnum_not_fou"
  );
  const labelRegNumb = fields.find((f) => f.name === "input_label");
  const labelRegNumb_Number = fields.find(
    (f) => f.name === "input_label_Nmber"
  );
  const errRegNumNotFoundMob = fields.find(
    (f) => f.name === "input_label_Nmber_err_N_F"
  );
  ...
  if (action.value === "SCALCULATEPOLIS") {
    ...
    if (autoInfo.length > 0) {
      ...
      labelRegNumb.visible = true;
      labelRegNumb_Number.visible = true;
      errRegNumNotFound.visible = false;
      errRegNumNotFoundMob.visible = false;
    } else {
      labelRegNumb.visible = false;
      labelRegNumb_Number.visible = false;
      errRegNumNotFound.visible = true;
    ...
    if (fieldName === "NHORSE_VEHICLE_POWER") {
      ...
      if (action.value > 999) {
        ...
        if (fieldNHORSE.state !== null) {
          fieldNKH.state = null;
          fieldNKH.error = "Мощность не указана";
        }
        fieldNHORSE.state = false;
        fieldNHORSE.error = "Значение должно быть от 1 до 999";
      } else if (!field.value) {
    ...
    if (fieldName === "NKH_VEHICLE_POWER") {
      ...
      if (action.value > 734.77) {
        ...
        if (fieldNKH.state !== null) {
          ...
          fieldNHORSE.error = "Мощность не указана";
        }
        fieldNKH.state = false;
        fieldNKH.error = "Значение должно быть от 1 до 734.77";
      } else if (!field.value) {
  ...
  // кастомная валидация ошибок полей
  if (action.name === "SVEHICLE_MODEL") {
    isVehicleModel.error = "Марка авто не указана";
  }
  ...
  if (action.value === "Item36585") {
    ...
    //console.log('Нажал на кнопку Рассчитать осаго')
