...
function isValidChallengeCookie(value) {
  ...
  if (tsPart.length < 7 || tsPart.length > 12) return false;

  if (randomPart.length < 6 || randomPart.length > 40) return false;
  ...
  if (tsMs > now + CLOCK_SKEW_MS) return false;
  if (now - tsMs > CHALLENGE_COOKIE_MAX_AGE_MS + CLOCK_SKEW_MS) return false;
