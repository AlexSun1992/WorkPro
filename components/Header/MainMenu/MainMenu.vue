    ...
    <b-modal ref="products" hide-footer hide-header>
      <div class="mb-4" v-for="(item, index) in products.sections" :key="index">
        ...
        <span v-for="(link, index) in item.links" :key="index">
          <b-button class="mr-4" :href="link.link.url">{{
            link.link.title
          }}</b-button>
        </span>
      </div>
      <b-button v-if="products.link" variant="link" :href="products.link.url">{{
        products.link.title
      }}</b-button>
    </b-modal>
    <CitySearch />
...
export default {
  ...
  computed: {
    ...
    phoneNumber() {
      return this.$store.state.pages.config.wpreso_settings_federal_number
        .default;
    },
  },
};
