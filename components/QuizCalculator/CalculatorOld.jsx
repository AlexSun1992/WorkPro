  ...
  switch (action.type) {
    case DEL_CHOSEN_ANSWER:
      ...
      for (i = 0; i < prevChosenAnswers.length; i++) {
        if (prevChosenAnswers[i].id == action.answer.id) {
          ...
          modifiedChosenAnswers = modifiedChosenAnswers.concat(
            prevChosenAnswers[i]
          );
        }
      }
      ...
      if (modifiedChosenAnswers.length === 0) {
        ...
        nextIssueId =
          modifiedChosenAnswers[modifiedChosenAnswers.length - 1].properties
            .nnext_issue;
      }
    ...
    case ADD_CHOSEN_ANSWER:
      nextQuestion = findNextQuestion(
        state.questions,
        action.answer.properties.nnext_issue
      );
      return {
        quizId: state.quizId,
        questions: state.questions,
...
class Calculator extends React.Component {
  ...
  render() {
    ...
    if (
      ...
      (Object.keys(this.props.currentQuestion).length === 0 &&
        this.props.currentQuestion.constructor === Object)
    ) {
      var firstQuestion = findNextQuestion(
      ...
      questionEl = (
        <Question id="question" question={this.props.currentQuestion} />
      );
      ...
      if (this.props.currentQuestion.properties.sshow_type === "SYSTEM_END") {
        ...
        calcResult = (
          <ErrorBoundary>
            <ConnectedCalcResultComponent key={Math.random()} />
          </ErrorBoundary>
        );
    ...
    console.log(
      "this.props.value.backgrounds = " + JSON.stringify(this.props.backgrounds)
    );
    ...
    return (
      <div>
        ...
        <div className="block-calc" id="calculator-item">
          <h2>{this.props.calcParams.stitle_h1}</h2>
  ...
  componentDidMount() {
    ...
    this.props.chosenAnswers.map((currentAnswer) => {
      var question = findNextQuestion(
        this.props.questions,
        currentAnswer.properties.idclient_quiz_issue
      );
      params +=
        "&" +
        question.properties.svalue_name +
        "=" +
        currentAnswer.properties.svalue_value;
    });
  ...
  render() {
    if (this.state.hasErrors === false) {
      ...
      var preloader = (
        <img
          key={Math.random()}
          src="/WarAgentResoRu/resources/images/newClientResoRu/2.gif"
          style={imgStyle}
        />
      );
      ...
      var fetchedResult = (
        <div>
          <span key={Math.random()} className="h1">
            {rub}
          </span>
          <span key={Math.random()}>.{kop} ₽</span>
          <span key={Math.random()} className="finish-price-text">
      ...
      if (
        ...
        link =
          "/WarAgentResoRu/newClientResoRu/auth/login.xhtml?welcome_id=wlc1799";
      }
      ...
      return (
        <div className="calc-alert-info row align-items-center">
          ...
          <div className="col-10">
            <span className="text-danger">Неполадки сети.</span>
            <br />
  ...
  render() {
    const listItems = this.props.answers.map((currentAnswer) => {
      if (
        ...
        currentAnswer.properties.lactive == 1
      ) {
    ...
    return <ul className="select-items">{filteredItems}</ul>;
  }
}
...
class NewCalc extends React.Component {
  ...
  render() {
    ...
    return (
      <a onClick={this.handleClick} style={cursorStyle}>
...
class Question extends React.Component {
  ...
  render() {
    ...
    if (this.props.question.properties.sshow_type === "select") {
      answers = (
        <ConnectedSelectAnswerComponent
          key={Math.random()}
          id="questionSelectAnswer"
          question={this.props.question}
        />
      );
    } else if (this.props.question.properties.sshow_type === "STRING") {
      answers = (
        <ConnectedTextAnswerComponent
      ...
      answers = (
        <ConnectedNumberAnswerComponent
          id="questionNumberAnswer"
          key={Math.random()}
          question={this.props.question}
        />
      );
      ...
      answers = (
        <ConnectedQuestionListAnswersComponent
          id="questionListAnswers"
          key={Math.random()}
          question={this.props.question}
        />
      );
    ...
    return (
      <div>
        <div className="calc-action-title">{this.props.question.name}</div>
...
class TextAnswer extends React.Component {
  ...
  render() {
    ...
    const textAnswer = this.props.answers.filter(function (currentAnswer) {
      if (
        currentAnswer.properties.idclient_quiz_issue == question_id &&
        currentAnswer.properties.lactive == 1
      ) {
...
class SelectAnswer extends React.Component {
  ...
  render() {
    const items = this.props.answers.map((currentAnswer) => {
      if (
        currentAnswer.properties.idclient_quiz_issue == this.props.question.id
      ) {
    ...
    return (
      <div className="d-table">
        ...
        <div className="d-table-cell">
          <a href="" className="next-button" onClick={this.handleClick()}>
            Далее
          </a>
        </div>
      </div>
    ...
    return (
      <ul className="select-finish-items" key={Math.random()}>
        {this.props.chosenAnswers.map((answer) => {
          if (answer !== undefined) {
            return <ChosenAnswer key={answer.id} answer={answer} />;
          }
        })}
      </ul>
    );
...
class QuestionAnswer extends React.Component {
  ...
  render() {
    if (
      this.props.value.properties.sli_css_class !== null &&
      this.props.value.properties.sli_css_class !== undefined
    ) {
...
class ChosenAnswer extends React.Component {
  ...
  render() {
    return (
      <li value={this.props.answer.id} key={Math.random()}>
        {this.props.answer.name}
        <button
          className="select-finish-items-del"
          onClick={this.handleClick}
        ></button>
      </li>
    );
...
var ConnectedCalculatorComponent = ReactRedux.connect(function mapStateToProps(
  state
) {
  return {
    quizId: state.quizId,
...
var ConnectedTextAnswerComponent = ReactRedux.connect(function mapStateToProps(
  state
) {
  return {
    quizId: state.quizId,
...
var ConnectedCalcResultComponent = ReactRedux.connect(function mapStateToProps(
  state
) {
  return {
    quizId: state.quizId,
