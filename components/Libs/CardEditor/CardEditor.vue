...
function eventHandler(data, item, action) {
  if (action === "displayText") {
    if (data.name === "FKSPOLICY") {
      return `${item.SSECONDNAME} ${item.SFIRSTNAME} ${item.STHIRDNAME}`;
    }
    if (data.name === "FKSSCHEDULE") {
      return item.SPERSON
        ? `${item.SNAME} Врач: ${item.SPERSON}`
        : `Нет данных`;
    }
    if (data.name === "FKSLPU") {
  ...
  if (item.name === "FKSSCHEDULE") {
    ...
    datetimeLabel.value = `${$nuxt
      .$moment(item.value.value["DDATE"])
      .format("DD.MM.YYYY")} ${item.value.value["DFROM"]} - ${
      item.value.value["DTO"]
    }`;
  }
